...
public class QuanLyHocVienView extends JPanel{
	...
	private JComboBox<String> cbHinhThucDong;
	...
	private void initUI() {
		...
		lblKhoaHoc.setFont(font);

		cbKhoaHoc = new JComboBox<>();
		cbKhoaHoc.setFont(font2);

		JLabel lblHinhThuc = new JLabel("HÌNH THỨC ĐÓNG:");
		lblHinhThuc.setFont(font);

		cbHinhThucDong = new JComboBox<>();
		cbHinhThucDong.setFont(font2);
		cbHinhThucDong.addItem("Đóng trọn khóa");
		cbHinhThucDong.addItem("Đóng theo tháng");
		...
		formnhap.add(JLabel_email); formnhap.add(JTextField_email1); formnhap.add(lblKhoaHoc); formnhap.add(cbKhoaHoc); formnhap.add(lblHinhThuc);
		formnhap.add(cbHinhThucDong);
		...
		timKiem.setForeground(new Color(47, 111, 219));
		...
		timKiem.add(mahv2); timKiem.add(JTextField_mahv3); timKiem.add(JButton_tim); timKiem.add(JButton_reset); timKiem.add(JButton_sapxep);
		
		
		
		DefaultTableModel_tablemodel = new DefaultTableModel(new String[] {"MA HOC VIEN", "TEN HOC VIEN", "MA LOP", "SDT", "EMAIL", "MA KHOA HOC", "HINH THUC DONG"}, 0);
		
		JTable_table = new JTable(DefaultTableModel_tablemodel);
		JTable_table.setAutoCreateRowSorter(false);
	...
	public JComboBox<String> getCbKhoaHoc(){
		return cbKhoaHoc;
	}
	public JComboBox<String> getCbHinhThucDong(){
		return cbHinhThucDong;
	}
	...
	public void loadKhoaHocToCombo() {
	    cbKhoaHoc.removeAllItems();
	    for (QuanLyKhoaHocModel kh : new KhoaHocDao().getAll()) {
	        cbKhoaHoc.addItem(kh.getMaKH());
	    }
	}
}
