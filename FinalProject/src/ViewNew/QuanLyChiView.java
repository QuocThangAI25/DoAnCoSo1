...
public class QuanLyChiView extends JPanel {
	...
	private void initUI() {
		...
		JLabel lblTitle = new JLabel("QUẢN LÝ CHI");
		...
		loai.setFont(font);
		cbloai = new JComboBox<>(new String[] {"Chi Lương Giáo Viên", "Chi Tiền In Tài Liệu"});
		cbloai.setFont(font1);
		JLabel magv = new JLabel("MÃ GIÁO VIÊN: ");
		magv.setFont(font);
		cbMaGV = new JComboBox<>();
		JLabel tenhv = new JLabel("TÊN GIÁO VIÊN: ");
		tenhv.setFont(font);
		...
		form.add(magd); form.add(JTextField_magd1); form.add(loai); form.add(cbloai); form.add(magv); form.add(cbMaGV); form.add(tenhv); form.add(txtTenHV);;
		...
		tablemodel = new DefaultTableModel(new String[] {"Mã Giao Dịch", "Loại", "Mã Giáo Viên", "Tên Giáo Viên", "Ngày", "Số Tiền", "Ghi Chú"}, 0);
	...
	public JTextField getMaGD() {
		return JTextField_magd2;
	}
	public JButton getXuatPdf() {
